...
# --------------------------------------------------------
# $Id: freemind_api.php 5359 2008-06-12 17:55:28Z dwarrine5 $
# --------------------------------------------------------
...
$t_core_dir = config_get_global( 'core_path' );
 ...
 *
 * @param $p_string  The content of the note to display.
 */
 ...
 *
 * @param $p_bug_data The issue data structure containing the issue data to be prepared for export.
 *
...
*
* @param $p_rows The issue rows that were returned from the filter api based on the active filter.
*/
function freemind_find_descendents( &$p_rows ) {
	...
	foreach ( $p_rows as $t_row ) {
		...
		$t_bug_list[$t_bug_id] = $t_bug;
		$t_bug_list[$t_bug_id]->id = $t_bug_id;
		$t_bug_list[$t_bug_id]->parents = array();
		$t_bug_list[$t_bug_id]->children = array();
		$t_bug_list[$t_bug_id]->related_to = array();
	}

	foreach ( array_keys( $t_bug_list ) as $t_bug_id ) {
		$t_relationships = relationship_get_all_dest( $t_bug_id );

		foreach ( $t_relationships as $t_relationship ) {
			if ( $t_relationship->type == BUG_DEPENDANT ) {
				if ( array_key_exists( $t_relationship->src_bug_id, $t_bug_list ) ) {
					array_push( $t_bug_list[$t_bug_id]->parents, $t_relationship->src_bug_id );
				}
			} else if ( $t_relationship->type == BUG_RELATED ) {
				array_push( $t_bug_list[$t_bug_id]->related_to, $t_relationship->src_bug_id );
			}
		}
		...
		foreach ( $t_relationships as $t_relationship ) {
			if ( $t_relationship->type == BUG_DEPENDANT ) {
				if ( array_key_exists( $t_relationship->dest_bug_id, $t_bug_list ) ) {
					$t_bug_list[$t_bug_id]->children[] = $t_relationship->dest_bug_id;
				}
			} else if ( $t_relationship->type == BUG_RELATED ) {
				array_push( $t_bug_list[$t_bug_id]->related_to, $t_relationship->dest_bug_id );
			}
		}
	}
